...
### 18:30 - refactor: consolidate field translations to validation attributes

**API Changes:**
- Added 25+ new field attributes to `validation.php` (en/es):
  - Common: `owner`, `elements`, `order`, `version`, `notes`, `city`, `sex`
  - Driver fields: `licenseNumber`, `licensePlate`, `licenseExpires`, `expirationDate`
  - Pricing fields: `baseFare`, `taxRate`, `tiers`, `minKm`, `maxKm`, `flatFee`, `perKmRate`, `distanceKm`, `subtotal`, `tax`, `distance`, `distanceFee`, `timeFee`, `surcharge`, `discount`
- Added corresponding cases to `Attributes.php` enum
- Removed duplicate field definitions from namespace files:
  - `businesses.php`: removed `owner`, `failed_to_load`, `detail.timestamps`
  - `catalogs.php`: removed `code`, `elements`, `failed_to_load`, `detail.description`, `detail.timestamps`, `detail.order`
  - `drivers.php`: removed `license_number`, `license_plate`, `license_expires`, `failed_to_load`, `detail.expiration_date`, `detail.timestamps`
  - `pricing.php`: removed field labels, column headers, calculator fields (now use validation attributes)
  - `quotes.php`: removed `failed_to_load`, `detail.version/notes/base_fare/distance_fee/time_fee/surcharge/discount/tax/timestamps`
  - `users.php`: removed `failed_to_load`, `detail.sex`, `detail.timestamps`
  - `addresses.php`: removed `label`, `city`, `failed_to_load`

**Admin Changes:**
- Synced types/generated.d.ts from API
- Regenerated runtime enums

**Note:** Admin pages still use namespace-specific translation keys with defaultValue fallbacks. Future task: update admin to use `validationAttribute()` helper for consistency.

### 17:55 - refactor: use crudSuccessMessage/crudErrorMessage helpers in all hooks

- Updated all mutation hooks to use standardized resource message helpers
- Files updated:
  - `useOrderMutations.ts` - approve, deny, delete
  - `useQuoteMutations.ts` - create, update, send, delete
  - `useDriverMutations.ts` - update, delete
  - `useBusinessMutations.ts` - update, delete
  - `useUserMutations.ts` - update, delete
  - `useCatalogElementMutations.ts` - create, update, delete
  - `useCurrencyMutations.ts` - update
  - `usePricingRuleMutations.ts` - create, update, delete (kept activate/clone with pricing namespace)
- Removed hardcoded translation strings in favor of `crudSuccessMessage(action, resourceKey)` and `crudErrorMessage(action, resourceKey)`
- All hooks now consistent with resource.php translation structure
