...
### 22:50 - fix: rate_date showing wrong day due to JS date parsing

- Fixed `formatDate` in currencies page to parse date-only strings as local time
- JS `new Date('2026-01-19')` parses as UTC midnight, causing off-by-one day in CR timezone
- Solution: append `T00:00:00` to date strings to parse as local time

### 21:51 - feat: individual filter clear options

- Added "All statuses", "All types", "All actions" options to Select dropdowns
- Users can now clear individual filters without using global "Clear filters"
- Date inputs show X button when filled to clear individually
- Converted Select values to use 'all' sentinel value mapped to empty string
